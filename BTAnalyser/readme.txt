...
==================== Installation (Compiling the Prototype) ====================


----- Lisp platforms -----

The prototype has been tested on SBCL (the preferred Lisp platform)
...
A 64-bit SBCL for Windows (invoked using sbcl.bat), 32-bit and
64-bit versions of CCL for Windows (invoked using w86cl.exe and
wx86cl64.exe respectively) and Linux (invoked using lx86cl and
lx86cl64 respectively) are included in the distribution.
...
Without changes, the working directory when running Lisp to
load and compile the prototype is assumed to be the source
directory of the prototype. This can usually be ensured by running
Lisp from the source directory of the prototype.
Otherwise you may need to set it via the Lisp variable
*default-pathname-defaults*, for example

  (setf *default-pathname-defaults* (truename "/home/username/prototype/"))

assuming the source directory is /home/username/prototype/.
An example using Windows would be

  (setf *default-pathname-defaults* (truename "C:/Users/username/prototype/"))
...
can be modified, e.g., in Windows, to
  ...
  (defvar *source-directory* "C:/Users/username/prototype/")

It would be convenient for subsequent usage of the prototype to
modify the line for load-all.lisp.


----- Compilation -----

Subdirectories for object files are required. Their names
correspond to the Lisp plaform (SBCL, GCL, CCL or ABCL).
For other Lisp platforms, the subdirectories must be created
and the files compile-lexicon.lisp, compile-all.lisp and
load-all.lisp must be modified accordingly.
...
BT Analyser now supports BESE files (in XML format).
The command for loading a BESE file is process-bese-file.
...
There is a function for computing a gap (subpath) between the an initial path
(path1) and a final subpath (path2):
