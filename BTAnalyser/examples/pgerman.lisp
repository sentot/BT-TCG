...
(process-bt-file "examples/PGerman160308.btc")
...
(defun generate-and-print-test-experimental (noi intermediates)
  (format t "~%Generating test paths with NOIs:~%")
  (loop for i in noi
        do (print-block-summary i))
  (loop for source in *blocks*
	...
	(loop for target in *blocks*
		...
		(format t "~%=======~%Filtered:~%")
		(print-test-paths-summaries
		 (shortest-paths-for-each-combination noi paths)))))))
...
(defun generate-and-print-test-latest (intermediates)
  (loop for i in intermediates
	...
	(loop for source in *blocks*
		...
		(print-block-summary-update-node target "Cache")
		(print-test-paths-summaries
		 (shortest-paths-for-each-combination (list i) paths)))))))))

(defun run-test-experimental (noi intermediates)
  (time (generate-and-print-test-experimental noi intermediates)))
...
;(run-test-experimental *all-noi* *all-actions*)

(time (generate-and-print-test-latest *latest-test-noi*))
